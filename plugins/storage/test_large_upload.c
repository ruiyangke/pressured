...
int main(int argc, const char *argv[]) {
  ...
  // Set expected size for Content-Length header (required for S3)
  // Use dlsym since s3_set_upload_size is in the plugin, not linkable directly
  // Must open plugin ourselves since plugin_manager doesn't use RTLD_GLOBAL
  void *plugin_handle = dlopen(S3_PLUGIN_PATH, RTLD_NOW | RTLD_NOLOAD);
  if (!plugin_handle) {
    // Plugin not yet loaded with NOLOAD, try regular open
